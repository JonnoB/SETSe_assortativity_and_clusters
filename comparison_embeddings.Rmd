...
This markdown file should be run after all chunks down to Introduction_to_sets2.Rmd has been run
...
embedded_files <- list.files(file.path(PLwd, "peel_benchamrk_embeddings"), full.names = T)
...
bind_rows(GEM_embeds_agg, n2v_embeds_agg, node_data_agg) %>%
  ...
  labs(title = "Seperating Peel's Quintet in 2 dimensions",
       x = "dimension 1 (Tension for SETSe)",
       y = "dimension 2 (elevation for SETSe)")
...
#accuracy plot
bind_rows(GEM_embeds, SETSe_embeds, node_perf) %>%
  filter(.metric =="accuracy") %>%
