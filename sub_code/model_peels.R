...
modelling_peels_df <- peel_strain_sum %>% ungroup %>%
  filter(k==0) %>%
  select(strain, elevation, graph_type) 

#create the formula to use
mod_form <- as.formula(graph_type ~ strain + elevation)

#create the ten sets of repeat folds
#make a large number of repeats to be able to get distribution
set.seed(4622)
rs_obj <- vfold_cv(modelling_peels_df, v = 10, repeats = 100)#, strata = graph_type)
...
log_holdout_results <- function(splits, ...) {
  ...
  holdout <- assessment(splits)
  # Fit the model to the 90%
  mod <- vglm(graph_type ~., family=multinomial, data = train_df)
