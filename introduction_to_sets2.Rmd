...
$ /usr/bin/time --verbose ./my_program
	...
	comments from Heatgher Kelly
	
	you replace 'my_program' with what you are trying to run (edited) 
5:01
and the $ is just the prompt
5:03
(Is also in the how to page in our docs, 'How do I estimate what resources to request in my jobscript?' and I think I linked it from the top of the example scripts page)
5:04
so if you're running R, you put your whole normal R runline after the --verbose
...
peel_strain_sum %>%
  filter(k==0) %>%
  ggplot(aes(x = mean_tension_per_node, y = mean_abs_elevation, colour = graph_type)) + geom_point(alpha = 0.8) +
...
test <- 1:length(bigraph$components) %>%
map_df(~tibble(number = .x, 
               present = 1024 %in% as.integer(bigraph$components[[.x]]),
               size = length(bigraph$components[[.x]])))
...
facebook_embeddings_data <- 1:length(file_paths) %>% #c(1,2,4,5) %>%
  map(~{
  ...
  facebook_embeddings_data$network_dynamics %>%
  bind_rows() %>%
  ...
  time_taken_iter <-left_join(facebook_embeddings_data$time_taken %>%
    bind_rows(),
  facebook_embeddings_data$bicomp_data %>%
    bind_rows() ) %>%
  ...
  time_taken__iter_mod <- lm(time_per_iter~node_x_edge, time_taken_iter)
...
tibble(lambda = box_cox_data_iter$x, log_likelihood = box_cox_data_iter$y) %>%
  ...
  time_taken_iter %>%
    ggplot(aes(x = nodes*edges, y = time_per_iter, colour = log10(edges*nodes))) + geom_point()+
    scale_color_viridis_c()
  

facebook_embeddings_data$node_detail %>%
    bind_rows() %>%
  filter(grepl("Simmons", file_name),
         (year %%1 == 0)) %>%
  ggplot(aes(x = (mean_tension), y = (elevation), colour = factor(year))) + geom_jitter()
...
time_taken_df <- facebook_embeddings_data$time_taken %>%
  bind_rows() %>%
  left_join(uni_stats,. , by = c("file_name")) %>%
  select(-nodes,-edges) %>%
  left_join(.,   facebook_embeddings_data$bicomp_data %>%
    bind_rows()) %>%
  mutate(diff = as.numeric(time_taken),
         diff2 = diff^(0.15),
         node_x_edge = nodes*edges
         ) %>%
  filter(nodes >10)
...
time_taken_df %>%
  ggplot(aes(x = nodes, y = (diff) )) + geom_point()+
  labs(title ="time to convergence is O(5) time relative to nodes or edge for social networks",
       subtitle = "however the data appear slightly heteroscedastic")
#The relationship is not exponential as can be demonstrated using a simple test on exponential vs quadratic toy data
time_taken_mod <- lm(diff~node_x_edge, time_taken_df)
summary(time_taken_mod)

#This model is much better
time_taken_mod2 <- lm(diff2~node_x_edge, time_taken_df)
  ...
  map_df(~{
    ...
    elevation_data <-test$embeddings_data$node_embeddings %>%
      summarise(mean_abs_elev = mean(abs(elevation)),
                ...
                max_abs_static_force = max(abs(static_force)),
                min_abs_static_force = min(abs(static_force)),
                range_static_force = max(static_force)-min(static_force),
                static_force = sum(abs(static_force)),
                students = n())
    
    strain_tension <- test$embeddings_data$edge_embeddings %>%
      summarise(
        sum_tension = sum(tension),
...
test %>% 
ggplot(aes(x = max_abs_static_force, y =static_force, color = edges))+ geom_point()+
   ...
   uni_pattern <- grep("Caltech", uni_files )
    ...
    g <- readRDS(uni_files[uni_pattern])  %>% #load file
      ...
      facebook_year_clean() %>% prepare_SETSe_continuous(., k = 1000, force_var = "year", 
                                                         sum_to_one = FALSE, 
                                                         distance = 1) %>%
      set_edge_attr(., "flow", value = 1)
    
    
    
    
      Prep2 <- SETSe_data_prep2(g = g, 
                          force = "force", 
                          distance = "distance", 
                          mass = 1, 
                          edge_name = "edge_name",
      ...
      head(Prep$non_empty_matrix) ;head(Prep2$non_empty_matrix)
      
      test <- Prep$non_empty_matrix %>% as_tibble
      test2 <- Prep2$non_empty_matrix %>% as_tibble
      ...
      all.equal(Prep$non_empty_matrix, Prep2$non_empty_matrix)
    
      
      
    embeddings_data <- auto_SETSe(g, tstep = 0.001, verbose = TRUE, sparse = F, sample = 100)

   bind_rows( embeddings_data$node_embeddings %>%
                mutate(type = "old"),
    embeddings_data2$node_embeddings %>%
               mutate(type ="new")
    
    ) %>%
     select(node, type, value = elevation) %>%
     pivot_wider(names_from = type, values_from = value) %>%
     ggplot(aes(x = old, y = new)) + geom_point()
  
   
   head(embeddings_data$node_embeddings$elevation)
    head(embeddings_data2$node_embeddings$elevation)
...
There is a bug it needs to be understood and fixed

```{r}

  #load common network
    g <- readRDS(uni_files[grep("Caltech", uni_files )])  %>% #load file
      remove_small_components()  %>%
            facebook_year_clean()%>%  
      prepare_SETSe_continuous(., k = 1000, 
                               force_var = "year", 
                               sum_to_one = FALSE, 
                               distance = 1)
...
#so simple so nice
    embeddings_data2 <- g  %>%
      auto_SETSe2(., tstep = 0.1, verbose = TRUE, sparse = T, sample = 100)
 
 #The old school method.. a pain in the adze
 embeddings_data <- g %>%
   set.edge.attribute(., "power_flow", value = 1) %>%
    set.edge.attribute(., "capacity", value = 1) %>%
   auto_SETSe(., tstep = 0.1, verbose = TRUE, sparse = T, sample = 100)

test <- bind_rows( embeddings_data2$node_embeddings %>% mutate(type = "new"),
  embeddings_data$node_embeddings %>% mutate(type = "old"))


  node_detail <- embeddings_data2$edge_embeddings %>% tibble() %>%
    ...
    mutate(is_round = (year%%1)==0,
           year = ifelse(is_round, year, NA))
  
  node_detail %>%
    filter(!is.na(year)) %>%
    ggplot(aes(x = mean_tension, y = elevation, colour = factor(year))) + geom_point()
  
  
  
  prep2 <-g %>%
    SETSe_data_prep2(., force = "force", distance = "distance", mass = 1, edge_name = "edge_name", k = "k", sparse = TRUE)

  prep <-  g %>%
   set.edge.attribute(., "power_flow", value = 1) %>%
    set.edge.attribute(., "capacity", value = 1) %>%
    SETSe_data_prep(., force = "force", distance = "distance", flow = "power_flow", mass = 1, edge_name = "edge_name", k = "k", sparse = T)
  
  
#identical  #kvect and #dvect can't be checked with this graph as the edges are all identical
  all.equal(prep$node_embeddings,
  prep2$node_embeddings)
  
  prep$node_embeddings$node[1:10]
  prep$ten_mat[1:10, 1:10] %>% row.names()
  prep2$ten_mat[1:10, 1:10] %>% row.names()
 vertex_attr(g, "name")

 as_adjacency_matrix(g, sparse = T) %>% {.[1:10, 1:10]} %>% row.names()
 
  as_data_frame(g, what = "vertices")$name[1:10]

  sparseA <- prep$ten_mat
  sparseB <- prep2$ten_mat
  class(sparseB)
all.equal(sparseA, sparseB)  

sparseA %>% rownames() %>% head
sparseB %>% rownames() %>% head
```
