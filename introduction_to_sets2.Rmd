...
library(e1071)
  ...
  list.files("/home/jonno/rSETSe/R", pattern = ".R", full.names = T) %>%
    walk(~source(.x))
```
...
hyper tolerance is 0.3 for all except for "UCSD34", "GWU54", "UC33", "Tennessee95"  which need hyper tolerance 0.1

92 98 99 100 


task id 98 UF21" has issues converging block 202. Appears to go into a loop
```{r}
...
#There appears to be errors in the embeddings, This is probably either a minor bug in the functions or I didn't remove the free nodes before embedding. This is to find out
how_many_nodes  <- readRDS(
  file.path(PLwd, "facebook_embeddings" , "processed_embeddings",
            "facebook_node_detail.rds" )) %>%
  group_by(file_name) %>%
  summarise(count = n(),
            static_force = sum(abs(static_force)),
            tens = mean(mean_tension),
            tens_b = mean(mean_tension, na.rm = T)) %>%
  left_join(HPC_time_process)

how_many_nodes %>%
  ggplot(aes(x = as.numeric(time), y = log10(count), colour = (static_force)>200)) +geom_point()


  HPC_node_details <- readRDS(
  file.path("/home/jonno/setse_1_data/facebook_embeddings/processed_embeddings",
            "facebook_node_detail.rds" )) %>%
  group_by(file_name) %>%
  summarise(static_force = sum(abs(static_force)),
            net_tension = mean(net_tension),
            median_net_tension = median(net_tension))

  
  
  HPC_node_details_test <- readRDS(
  file.path("/home/jonno/setse_1_data/facebook_embeddings/processed_embeddings",
            "facebook_node_detail.rds" )) %>%
  filter(file_name == "Reed98")

HPC_mem <- readRDS(
  file.path("/home/jonno/setse_1_data/facebook_embeddings/processed_embeddings_1000",
            "facebook_memory_df.rds" ))

HPC_nodes <- readRDS(
  file.path("/home/jonno/setse_1_data/facebook_embeddings/processed_embeddings_1000",
            "facebook_node_embeddings.rds" ))%>%
  filter(file_name == "Rochester38")

HPC_test <- readRDS(
  file.path("/home/jonno/setse_1_data/facebook_embeddings/processed_embeddings_1000",
            "facebook_time_taken_process.rds" )) %>%
  mutate(diff = difftime(stop_time,start_time))
...
HPC_dyn <- readRDS(
  ...
  filter(Iter !=0) %>%
  group_by(file_name) %>%
  mutate(relative = static_force/first(static_force),
...
test <- HPC_dyn %>%
  #filter(max_rel >1,
  #       Iter <5000) %>%
  left_join(how_many_nodes %>% select(file_name, count)) %>%
  filter((count %in% unique(count)[rank(unique(count))<=5])) #get the 5 smallest uni's


test %>%
  ggplot(aes(x = t, y = log10(relative), color = file_name)) + 
  geom_line() 
 # coord_cartesian(ylim = c(-5, 1.1))# +
 # scale_color_brewer(type = "qual")

completed_unis <- list.files("/home/jonno/setse_1_data/facebook_embeddings/facebook", pattern = ".rds") %>%
  str_remove(., ".rds")

missing <- uni_stats %>%
  mutate(rank = rank(nodes)) %>%
  filter(!(file_name %in% completed_unis))

embeddings_data <- readRDS(
  file.path("/home/jonno/setse_1_data/facebook_embeddings/facebook1000",
            "Texas84.rds" ))$embeddings_data


HPC_time_process <- readRDS(
  file.path("/home/jonno/setse_1_data/facebook_embeddings/processed_embeddings",
            "facebook_time_taken_process.rds" )) %>%
  mutate(time = difftime(stop_time, start_time, units = "mins"))
...
HPC_drag_vals %>%
  ggplot(aes(x = log10(nodes), y = common_drag_iter))+ geom_point()
"UIllinois20.rds"
g <- readRDS(file.path("/home/jonno/setse_1_data/facebook100/facebook100_igraph", "Caltech36.rds"))  %>% #load file
  remove_small_components()  %>%
  facebook_year_clean() %>% prepare_SETSe_continuous(., node_names = "name", k = 1000, force_var = "year", 
                                                     sum_to_one = FALSE, 
                                                     distance = 100) 

test_df <- readRDS(file.path("/home/jonno/setse_1_data/facebook100/facebook100_igraph", "Haverford76.rds"))  %>%
...
profvis(create_balanced_blocks2(g, 
                               force = force,
                               bigraph = bigraph))


system.time(test1 <- create_balanced_blocks2(g, 
                               force = force,
                               bigraph = bigraph))

system.time(test2 <- create_balanced_blocks(g, 
                               force = force,
                               bigraph = bigraph))
...
1:12 %>%
  map(~{
    ...
    all.equal(as_data_frame(test1[[.x]], what = "vertices"), as_data_frame(test2[[.x]], what = "vertices"))
...
start_time <- Sys.time()
embeddings_data <- SETSe_shift(
  g = balanced_blocks[[OriginBlock_number]],
  force = "force",
  distance = "distance",
  edge_name = "edge_name",
  k = "k",
  tstep =0.01,
  mass = ifelse(is.null(mass), 
                mass_adjuster(balanced_blocks[[OriginBlock_number]], 
...
g_vertices <- as_data_frame(g, what = "vertices")

start_time <- Sys.time()
embeddings_data2 <- SETSe(
  g = balanced_blocks[[OriginBlock_number]],
  force = "force",
  distance = "distance",
  edge_name = "edge_name",
...
tstep_change <- 0.5
    ...
    tests_mem <- embeddings_data$memory_df
    test_nodes <- embeddings_data$node_embeddings
    sum(abs(test_nodes$static_force))
    tests_mem <- embeddings_data$memory_df %>%
      filter(complete.cases(.))
    test_edges <- embeddings_data$edge_embeddings
    test_dyn <- embeddings_data$network_dynamics %>%
      mutate(relative = static_force/first(static_force),
             t_diff = t-lag(t)) 
    
        test_dyn2 <- embeddings_data2$network_dynamics %>%
      mutate(relative = static_force/first(static_force),
             t_diff = t-lag(t)) 
    #test_dyn %>%
        bind_rows(test_dyn %>% mutate(type = "shift"),
                  test_dyn2 %>% mutate(type = "base")) %>%
      mutate(relative = static_force/first(static_force)) %>%
        #   filter(Iter>28000) %>%
      ggplot(aes(x = Iter, y = log10(relative), colour = type)) + geom_line()
...
test_edges <- as_data_frame(g, what = "edges")
test_vertices <- as_data_frame(g, what = "vertices")

node_ids <- test_g$vertices$name

test_vertices[test_vertices$name != CurrArt,]

test_edges[test_edges$from != CurrArt & test_edges$to != CurrArt,]

which(get.vertex.attribute(g, "name") ==CurrArt)
      ...
      HPC_test <- readRDS(
        file.path("/home/jonno/setse_1_data/facebook_embeddings/processed_embeddings",
                  "facebook_time_taken.rds" )) %>%
        mutate(diff = (stop_time-start_time)/60) %>%
        filter(file_name == "Texas80")
      
      HPC_test_auto <- readRDS(
  ...
  file.path("/home/jonno/setse_1_data/facebook_embeddings/processed_embeddings",
            "facebook_node_embeddings.rds" )) %>%
        group_by(file_name) %>%
        summarise(stat_force = sum(abs(static_force))) %>%
        left_join(     readRDS(
  file.path("/home/jonno/setse_1_data/facebook_embeddings/processed_embeddings_auto",
            "facebook_node_embeddings.rds" )) %>%
        group_by(file_name) %>%
      ...
      test <- readRDS(
  file.path("/home/jonno/setse_1_data/facebook_embeddings",
            "facebook_node_embeddings.rds" )) %>%
        group_by(file_name) %>%
        summarise(static_force = sum(abs(static_force)))
...
process_test <- readRDS(
  file.path("/home/jonno/setse_1_data/facebook_embeddings/processed_embeddings",
            "facebook_time_taken_process.rds" )) %>%
  mutate(bicomp_minutes = (stop_time-start_time)/60) %>%
  select(file_name, bicomp_minutes) %>%
  left_join(readRDS(
  file.path("/home/jonno/setse_1_data/facebook_embeddings/processed_embeddings_auto",
            "facebook_time_taken_process.rds" )) %>%
  mutate(auto_minutes = (stop_time-start_time)/60) %>%
  select(file_name, auto_minutes) ) %>%
  mutate(diff = auto_minutes-bicomp_minutes) %>%
  left_join(uni_stats)
