...
relate_figure <-  node_details_df %>%
    ...
    labs(title = "The average elevation and tension embeddings of the facebook 100 dataset.",
         y = "absolute mean elevation per node",
         x = "The mean of the per node tension",
         color = "class\nassortativity")
...
node_details_df %>% ggplot(aes(x = student_assort)) + geom_density()
  ...
  assort_mod <- lm(student_assort ~ abs_elevation + mean_tension + euc_tension, node_details_df)
  summary(assort_mod)
  #the linear model predicting gender assortyativity from year is nothing
    lm(gen_assort ~ year_assort, node_details_df) %>% summary
...
#actuall time per iteration seems to be linear to the number of edges
  time_taken_df %>%
...
time_taken_df %>%
  ggplot(aes(x = edges*log(edges), y = time_taken )) + geom_point()+
...
#          year %in% 2004:2005
 ...
 #  filter(year %in% c(2004, 2005, 2006)) %>%
   ggplot(aes(x = log(euc_tension), y = elevation, color = factor(year))) + geom_point()+
...
#means doesn't seem very useful here, probably becuase so much is going on
 fb_metrics <- metric_set(accuracy, kap, bal_accuracy)
...
#  filter(metric== "bal_accuracy") %>%
  ggplot(aes(x = factor(k), y = diff, fill = metric)) + geom_boxplot()+
  facet_grid(metric~active_period)
